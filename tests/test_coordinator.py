...
from custom_components.plum_ecomax.coordinator import MAX_DELTA_REJECTIONS, PlumDataUpdateCoordinator
...
class TestMaxDeltaRecovery:
    ...
    def test_min_max_violations_are_never_force_accepted(self, coordinator):
        ...
        for _ in range(MAX_DELTA_REJECTIONS + 5):
            valid, val = coordinator._validate_value(slug, 60.0, 36.8)  # above max=50, not the 999 sentinel
            assert valid is False
            assert val is None
